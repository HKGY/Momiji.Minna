...
var arg = {
	...
	authorId &65536解析markdown &131072预览模式
	c 要解析内容的元素
	txt 或者要解析的文字
			注意文字中遇到换行'\n'时意为bbscode分段 所有已开始的tag应被强制结束
	noImg &1减少图/视频/内嵌 &2不显示图 &524288附件使用小缩略图 &1048576...使用中等缩略图 &2097152不显示表情
	fId
	authorId
	rvrc 无用
	isSig 是签名
...
this.attach.srcSelect(a,
	((arg.opt&(16|2)) ? 2 : 0)
	| (this.imgGenPerPostCounter[arg.pid]>100 ? 1 : 0)
	)
console.log(a)
if (
	(a.type=='img' || a.type=='video')
	&& ((arg.opt & 128) ? this.checkSigImg(src) : (a.self==2))
	){//附件or签名upload
	...
	if(a.type!='img'){
		if(a.type=='video' && (a.ext=='mp.mp4'||a.ext=='gif.mp4'))
			return '[flash=video]'+(a.org?a.org:a.init)+'[/flash]'
		return src
		}
	
	var dw=0,dh=0,dr = (a&&a.w&&a.h)?a.h/a.w:0
	if(a.org)
		arr['data-srcorg'] = a.org
	arr.src = a.init
	arr['data-iw'] = a.iw
	arr['data-ih'] = a.ih
	arr['data-nw'] = a.w
	arr['data-nh'] = a.h
	if((arg.opt&(16|2))==0 && this.attach.less)
		arr['data-zom'] = 1

	if(!a.inThumb && a.size>512)
		this.imgGenPerPostCounter[arg.pid]++
/*
	...
	if(tmp){//用户指定使用缩略图
		...
		if(a.type=='img' && a.w && a.w<=dw && a.size && a.size<200 && (noimg&(1|524288|1048576))==0){//原图和缩略图一样大 并且小于200k 使用原图
			src = arr['data-srcorg']
			dw = a.w
			dh = a.h
			}
...
*/
	...
	if(tmp = this.previewGetUrl(arg, arr.src))
		arr.src = arr['data-srcorg']= tmp
	}
else{//不是附件
...
if(window.__APPEMBED){
	arr['data-apporg']=a.org
	arr['data-appinit']=a.init
	this.insertAllImg(arg.i, a.org, a.init, a.type, a.orgs)
	}
...
if(!a.btn){
	...
	arr[window.__INSECTOB ? 'data-srclazy' : 'data-srcnolazy'] = arr.src
	arr.src='about:blank'
...
ubbcode.imgGenPerPostCounter={}

ubbcode.imgGenJoin = function(arr){
var img = "<img "
//if(arr.iw)arr.style+=';width:'+arr.iw+'px'
//if(arr.ih)arr.style+=';height:'+arr.ih+'px'
for(var k in arr)
...
var h , w 
if(h=(o.getAttribute('data-ih')|0)){
	var w = o.getAttribute('data-iw')|0, aw = this.getAvilWidth(o)
	if(w>aw){
		if(o.getAttribute('data-zom'))
			aw = aw*0.75
		h = aw/w*h, w=aw
		}
...
if(o.getAttribute('data-srclazy'))
	...
	o.src=o.getAttribute('data-srcnolazy')
/*
		...
		if(window.__APPEMBED){
			if(s = this.getAttribute('data-apporg')){
				ubbcode.embedCallOpenPic(
					...
					,s
					,this.getAttribute('data-appinit'))
...
ubbcode.embedCallOpenPic = function(argi,ourl,initurl){
...
ubbcode.insertAllImg = function(i,urlorg,urlinit,type,urlstatic){//type: video / img
var tmp
...
ubbcode.previewGetUrl=function(arg,attachUrl){
if(arg.opt & 131072){
	if(arg.waitApplyAttch){
		var dsr = arg.waitApplyAttch[ attachUrl.replace(/^.*(mon_\d{6}\/\d{2}\/[^\.]+)\..+$/,'$1') ]
		if(dsr)
			return dsr
		}
	}
...
arg.txt = arg.txt.replace(/\[(flash|media)(?:=(video|audio))?\](.{1,512}?)\[\/\1\]/gi,function($0,$1,$typ,$2){
	...
	if(commonui.ifUrlAttach(u)){
		...
		if(tvb&25){//1 8 16
			var tmb = self.attach.thumbPx(att.thumb),tmp,tms=att.url
			tmb = tmb ? (att.url+tmb) : ''
			if(tmp = self.previewGetUrl(arg,tmb)){
				tmb = tmp
				tms = ''
				}
			var vo = "<video playsinline preload=none muted=true src='"+tms+"' "+(tvb==1?'loop':'')+" poster='"+tmb+"' "+((tvb&32)?"data-playlazy='1'":'controls')+" style='display:none;background:"+__COLOR.gbg1+";width:"+(att.w?att.w:200)+"px;height:"+(att.h?att.h:200)+"px' data-argi='"+arg.i+"' onclick='ubbcode.autoplayVideo(event,this)' onplay='ubbcode.autoplayVideo(event,this)' onended='ubbcode.autoplayVideo(event,this)' onpause='ubbcode.autoplayVideo(event,this)'></video><img src='about:blank' style='display:none' onerror='ubbcode.videoAdjSize(this.previousSibling)'/>"
			if(window.__APPEMBED)
				self.insertAllImg(arg.i,att.url,tmb,'video',(att.thumb&128) ? (att.url+'.org.jpg'):'')
			var srs = self.attach.srcSelect(att)
			if(srs.btn){ 
				tmp = self.randDigi('manualLoadImg', 10000)
				self.manualLoadCache[tmp]=vo
				return "<button type='button' onclick='this.nextSibling.innerHTML=ubbcode.manualLoadCache[\""+tmp+"\"];this.nextSibling.style.display=\"\";this.style.display=\"none\"'>显示图片</button><span style='display:none'></span>"
				}
			else
				return vo
			}
...
var _ATTACH_LIB = {
px:'https://img.nga.178.com/attachments'
,patt:/\/([a-zA-Z0-9_\-]+)\.([a-zA-Z0-9_\-]+(?:\.mp4)?)(?:\.(thumb|thumb_s|thumb_ss|medium|org|t)\.[a-zA-Z0-9]+)?$/
,pdata:/^-?[a-z0-9]+Q[a-z0-9\-]+([KZXM][a-z0-9]+)?(?:T([a-z0-9]+))?(?:S([a-z0-9]+)-([a-z0-9]+))?/
,pthumb:/\.(thumb|thumb_s|thumb_ss|medium)\.(jpg|jpeg)$/
,pho:/^\.?\/?mon_\d+/
,pspx:/^.*\.?\/?(mon_\d+\/\d+\/)/
,no:false
,wifi:1
...
a.self = this.ifUrlAttach(src)
if(a.self==2){
	if(n = src.match(this.patt)){
		...
		a.autoUrl = this.px+'/'+src.replace(this.pspx,'$1')
		}
	else{
		a.autoUrl = this.px+'/'+src.replace(this.pspx,'$1')
		n = a.autoUrl.match(/\.([a-z]{3,4})$/)
		if(n && this.parseExt(n[1])=='img'){
			n = this.stripThumb(a.autoUrl).match(/\.([a-z]{3,4})$/)
			if(n)
				a.type = this.parseExt(n[1])
			}
		}
	}
else
...
,ifUrlAttach:function(u){
if(u.match(this.pho))return 2
if(window.commonui)
	return commonui.ifUrlAttach(u)
...
return s.replace(this.pthumb,'')
},//
orgShoot:function(s){
return s.replace(this.pthumb,'')+'.org.jpg'
},//
...
,
/**
 * @param {string | AttachO} a
 * @param {int} opt &1hi &2max
 * @returns {AttachO}
 */
srcSelect : function(a,opt){
if(typeof a =='string')
	a = this.parseName(a)
if(this.no===false)
	this.init()
if(a.type=='img' || a.type=='video'){
	if(this.no)
		a.btn = a.size
	if(!a.isThumb){
		a.init = a.autoUrl
		...
		if((this.max || opt&2) && (opt&1)==0){
			...
			if(this.hi || (opt&1)){
				if(a.size>60 || !a.size){
					if(a.thumb & 64){
						a.init = a.autoUrl+'.medium.jpg'
						a.inThumb = 'medium'
						a.iw = 640
						a.ih = 640*a.h/a.w
						}
					else if((a.thumb&65568)==65568){
						a.init = a.autoUrl+'.thumb.jpg'
						a.inThumb = 'thumb'
						a.iw = 320
						a.ih = 320*a.h/a.w
						}
		...
		if(a.type=='video' && a.init == a.autoUrl)
			a.init = a.autoUrl+'.org.jpg'
		}
	else{
		a.init = a.autoUrl
		var dw= this.thumbWidth(a.isThumb)
		a.iw = dw
		a.ih = dw*a.h/a.w
		a.inThumb = a.isThumb
		if(this.max){
			if(dw && a.w<=dw && a.size && a.size<200 && !this.no){
				a.init = this.stripThumb(a.autoUrl)
				a.inThumb = ''
				}
			}
		}
...
,thumbWidth:function(n){
var w
if(n=='medium')d = 640
else if(n=='thumb')w = 320
else if(n=='thumb_s')w = 130
else if(n=='thumb_ss')w = 60
else w = 320
return w
}//
,init:function(){this.no=this.no|0}
}////
...
init : function(){
var b = __SETTING.bi, s=this
s.no = b.noPic ? 1 : 0
s.hi = b.autoPicM ? 1 : 0
s.lo = b.autoPic ? 1 : 0
s.max = (this.hi || this.lo) ? 0 : 1
s.less = b.lessPic ? 1 : 0
},//

check:function(id,src){
/** @type {typeof _ATTACH_LIB & typeof _ATTACH_LIB_EXT} */
var s=this,
aa = s.parseName(src), x
if(aa.cacheKey && (x=this.cache['attach'+id]) && (x=x.index[ aa.cacheKey ])){
	x.ck=true
	}
return aa
},//
...
if(id===undefined){
	if(id = o.id.match(/(\d+)$/))
		...
		id = ubbcode.randDigi('attach_noid', 10000)
	}
else
	id = 'attach'+id
...
ubbcode.attach = Object.assign({}, _ATTACH_LIB, _ATTACH_LIB_EXT)
